...
const generateInvoicePDFBuffer = (order, shop) => {
  return new Promise((resolve, reject) => {
    ...
    const doc = new PDFDocument({ margins: { top: 40, bottom: 30, left: 50, right: 50 }, size: 'LETTER' });
    const buffers = [];
    ...
    // Customer Permit and License details in Footer
    let permitText = `Customer Seller Permit: ${shop.seller_permit || 'N/A'}`;
    if (shop.tobacco_license) {
      permitText += `  |  Customer Tobacco License: ${shop.tobacco_license}`;
    }
    doc.fillColor('#475569').fontSize(8).font('Helvetica-Bold').text(permitText, 50, footerTop + 8, { align: 'center', width: 512 });
    
    doc.fillColor('#94a3b8').fontSize(8).font('Helvetica').text('Thank you for your business', 50, footerTop + 20, { align: 'center', width: 512 });
    
    const genTime = formatCA_DateTime(new Date());
    doc.fillColor('#cbd5e1').text(`Generated by Woodland Distributors on ${genTime}`, 50, footerTop + 32, { align: 'center', width: 512 });
